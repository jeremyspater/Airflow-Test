...
A = read.csv(file = '/Users/jeremyspater/Dropbox/duke/methods/python for finance/github repository/airflow_updating_test/shiny airflow updating test/data/airflow_test_data.csv')

A[nrow(A)+1, 'Date'] = as.character(Sys.time())
A[nrow(A), 'Value'] = rnorm(1)

write.csv(A, file = '/Users/jeremyspater/Dropbox/duke/methods/python for finance/github repository/airflow_updating_test/shiny airflow updating test/data/airflow_test_data.csv',
            row.names=FALSE) #this prevents 'X' columns from getting added on the left
